...
from Auxiliary.Shuffle import Shuffle_Triple, Shuff_Fivefold

COMPARE = {'RSA': 'AttentionFinal', 'RLA': 'AttentionFinal', 'RMA': 'AttentionWeight_Final'}
POSITION = {'RSA': 18, 'RLA': 18, 'RMA': 24}


class MultiCNNWithAESR(NeuralNetwork_Base):
    def __init__(self, trainData, trainSeq, trainLabel, sentenceData, interviewData, transformWeight, firstAttention,
                 firstAttentionName, firstAttentionScope, secondAttention, secondAttentionName, secondAttentionScope,
                 sentenceFlag=True, interviewFlag=True, convSize=2, rnnLayers=1, hiddenNodules=128, batchSize=32,
        ...
        self.sentenceData, self.interviewData = sentenceData, interviewData
        self.sentenceFlag, self.interviewFlag = sentenceFlag, interviewFlag
        ...
        super(MultiCNNWithAESR, self).__init__(
    ...
    def BuildNetwork(self, learningRate):
        ...
        self.sentenceInput = tensorflow.placeholder(dtype=tensorflow.float32, shape=[None, 3200], name='sentenceInput')
        self.interviewInput = tensorflow.placeholder(dtype=tensorflow.float32, shape=[1, 12800], name='interviewInput')
        ...
        self.parameters['AttentionResultConcat_SR'] = tensorflow.concat(self.parameters['AttentionResultCurrent_SR'],
                                                                        axis=1)
        ...
        for sample in self.convSize:
            ...
            self.parameters['AttentionLoss'] = self.parameters['AttentionLoss'] + tensorflow.losses.absolute_difference(
                labels=self.parameters['AttentionMechanism_%d_SR' % sample][COMPARE[self.firstAttentionName]],
                predictions=self.parameters['AttentionMechanism_%d' % sample][COMPARE[self.firstAttentionName]])

        if self.sentenceFlag:
            self.parameters['SentenceTreatment'] = tensorflow.layers.dense(
                inputs=self.sentenceInput, units=128, activation=tensorflow.nn.relu, name='SentenceTreatment')
            self.parameters['AttentionResultCurrent'].append(self.parameters['SentenceTreatment'])
        ...
        if self.interviewFlag:
            self.parameters['InterviewTreatment'] = tensorflow.layers.dense(
                inputs=self.interviewInput, units=256, activation=tensorflow.nn.relu, name='InterviewTreatment')
            self.parameters['BLSTM_Result'] = tensorflow.concat(
                [self.parameters['BLSTM_AttentionMechanism']['FinalResult'], self.parameters['InterviewTreatment']],
                axis=1, name='BLSTM_Result')
        else:
    ...
    def Valid(self):
        result = self.session.run(
            ...
            feed_dict={self.dataInput: self.data[0], self.dataSeqInput: self.dataSeq[0],
                       self.labelInput: numpy.reshape(self.label[0], [1, 1]), self.sentenceInput: self.sentenceData[0],
                       self.interviewInput: self.interviewData[0:1]})
        print(result)
        print(numpy.shape(result))

    def Train(self, logName):
        trainData, trainLabel, trainSeq, trainSentence, trainInterview = Shuff_Fivefold(
            self.data, self.label, self.dataSeq, self.sentenceData, self.interviewData)
            ...
            for index in range(numpy.shape(trainData)[0]):
                loss, _ = self.session.run(
                    ...
                    feed_dict={self.dataInput: trainData[index], self.dataSeqInput: trainSeq[index],
                               ...
                               self.sentenceInput: trainSentence[index],
                               self.interviewInput: trainInterview[index:index + 1]})
                print('\rTrain %d/%d Loss = %f' % (index, numpy.shape(trainData)[0], loss), end='')
                totalLoss += loss
                file.write(str(loss) + '\n')
        return totalLoss

    def Test(self, savepath, testData, testLabel, testSeq, testSentence, testInterview):
            ...
            for index in range(numpy.shape(testData)[0]):
                predict = self.session.run(
                    ...
                    feed_dict={self.dataInput: testData[index], self.dataSeqInput: testSeq[index],
                               self.sentenceInput: testSentence[index],
                               self.interviewInput: testInterview[index:index + 1]})
