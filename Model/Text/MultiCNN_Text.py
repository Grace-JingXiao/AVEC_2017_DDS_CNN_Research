...
class MultiCNN_Text(NeuralNetwork_Base):
    def __init__(self, trainData, trainSeq, trainLabel, firstAttention, firstAttentionName, firstAttentionScope,
        ...
        super(MultiCNN_Text, self).__init__(
    ...
    def BuildNetwork(self, learningRate):
        self.dataInput = tensorflow.placeholder(dtype=tensorflow.int32, shape=[None, 25], name='DataInput')
        ...
        for sample in self.convSize:
            self.parameters['Layer1st_Conv_%d' % sample] = tensorflow.layers.conv2d(
                inputs=self.parameters['EmbeddingData'][:, :, :, tensorflow.newaxis], filters=8,
            ...
            self.parameters['Layer3rd_Reshape_%d' % sample] = tensorflow.reshape(
                tensor=self.parameters['Layer2nd_Conv_%d' % sample], shape=[-1, 25, 256 * 16],
                name='Layer3rd_Reshape_%d' % sample)

            self.parameters['AttentionMechanism_%d' % sample] = self.firstAttention(
                dataInput=self.parameters['Layer3rd_Reshape_%d' % sample], seqInput=self.seqInput,
                scopeName=self.firstAttentionName + '_Frame_%d' % sample, hiddenNoduleNumber=256 * 16,
